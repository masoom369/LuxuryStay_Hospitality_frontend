...
import { format, addDays, isToday, isAfter } from "date-fns";
...
const RoomAvailability = () => {
  ...
  const [rooms, setRooms] = useState([
    { id: 1, roomNumber: '101', roomType: 'Standard', status: 'available', floor: 1, price: 150, bedType: 'Double', maxOccupancy: 2, amenities: ['WiFi', 'TV', 'AC'] },
    { id: 2, roomNumber: '102', roomType: 'Standard', status: 'occupied', floor: 1, price: 150, bedType: 'Twin', maxOccupancy: 2, amenities: ['WiFi', 'TV', 'AC'] },
    { id: 3, roomNumber: '103', roomType: 'Deluxe', status: 'cleaning', floor: 1, price: 250, bedType: 'King', maxOccupancy: 2, amenities: ['WiFi', 'TV', 'AC', 'Minibar', 'Balcony'] },
    { id: 4, roomNumber: '201', roomType: 'Deluxe', status: 'available', floor: 2, price: 250, bedType: 'Queen', maxOccupancy: 2, amenities: ['WiFi', 'TV', 'AC', 'Minibar', 'Balcony'] },
    { id: 5, roomNumber: '202', roomType: 'Executive', status: 'maintenance', floor: 2, price: 300, bedType: 'King', maxOccupancy: 2, amenities: ['WiFi', 'TV', 'AC', 'Safe', 'Kitchen'] },
    { id: 6, roomNumber: '203', roomType: 'Executive', status: 'available', floor: 2, price: 300, bedType: 'King', maxOccupancy: 2, amenities: ['WiFi', 'TV', 'AC', 'Safe', 'Kitchen'] },
    { id: 7, roomNumber: '301', roomType: 'Suite', status: 'available', floor: 3, price: 500, bedType: 'King', maxOccupancy: 4, amenities: ['WiFi', 'TV', 'AC', 'Minibar', 'Bathroom', 'Balcony'] },
    { id: 8, roomNumber: '302', roomType: 'Suite', status: 'occupied', floor: 3, price: 500, bedType: 'King', maxOccupancy: 4, amenities: ['WiFi', 'TV', 'AC', 'Minibar', 'Bathroom', 'Balcony'] },
    { id: 9, roomNumber: '401', roomType: 'Presidential', status: 'available', floor: 4, price: 800, bedType: 'King', maxOccupancy: 6, amenities: ['WiFi', 'TV', 'AC', 'Minibar', 'Bathroom', 'Balcony', 'Kitchen', 'Living Room'] },
    { id: 10, roomNumber: '402', roomType: 'Presidential', status: 'reserved', floor: 4, price: 800, bedType: 'King', maxOccupancy: 6, amenities: ['WiFi', 'TV', 'AC', 'Minibar', 'Bathroom', 'Balcony', 'Kitchen', 'Living Room'] },
  ]);
  ...
  useEffect(() => {
    ...
    if (filter.roomType !== 'all') {
      filtered = filtered.filter(room => room.roomType.toLowerCase() === filter.roomType.toLowerCase());
    }

    // Apply filter by status
    if (filter.status !== 'all') {
      filtered = filtered.filter(room => room.status.toLowerCase() === filter.status.toLowerCase());
    }
              ...
              <div key={room.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                ...
                <div className="space-y-1 mb-3">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-bold text-accent">${room.price}/night</span>
                  </div>
                </div>
                ...
                <div className="mt-4 flex space-x-2">
                  ...
                  {(room.status === 'cleaning' || room.status === 'maintenance') && (
                    <button 
                      className="flex-1 bg-gray-200 text-gray-800 hover:bg-gray-300 transition-colors py-2 px-4 rounded-md text-sm"
                      onClick={() => console.log(`Mark room ${room.roomNumber} as available`)}
                    >
                      Mark Available
                    </button>
                  )}
        ...
        <div className="bg-white p-6 rounded-lg shadow-md border border-gray-100">
          ...
          <div className="space-y-4">
            {[1, 2, 3, 4].map(floor => {
              ...
              const occupiedRooms = floorRooms.filter(room => room.status === 'occupied' || room.status === 'reserved').length;
                ...
                <div key={floor} className="flex items-center">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-sm text-gray-600">{occupiedRooms} of {floorRooms.length} rooms occupied</span>
                      <span className="text-sm font-medium text-gray-900">{floorOccupancy}%</span>
                    </div>
